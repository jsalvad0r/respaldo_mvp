...
export function mapPolicyToColaborador(row: EmployeePolicyRow): Colaborador {
  ...
  return {
    ...
    email: row.email ?? '',
    notas: row.notas ?? '',
    numeroDocumento: row.colaborador_documento ?? '',
    fechaNacimiento: row.colaborador_fecha_nacimiento ?? '',
    padronCompleto: Boolean(row.colaborador_documento && row.colaborador_fecha_nacimiento),
